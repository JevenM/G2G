...
def train_adv(node, args, logger, round, sw = None, epo=None):
    ...
    for images, labels in train_loader:
        ...
        feature, _, out = node.cl_model(images)
...
def train_ssl(node, args, logger, round, sw=None):
    ...
    for k in trange(args.simclr_e):
        ls = 0
        running_loss_t, running_loss_f, running_loss_ls = 0.0, 0.0, 0.0
        loss = 0
        for iter_, (real_images, labels) in enumerate(train_loader):
            ...
            # loss = loss_ce_f + loss_ce_true + 2 * ls_
            loss = loss_ce_f + 2 * ls_
            ...
            node.optm_cl.step()
            ls += loss.item()
        sw.add_scalar(f'Train/t_loss/{node.num}', running_loss_t / len(train_loader), round*args.simclr_e+k) # type: ignore
        sw.add_scalar(f'Train/f_loss/{node.num}', running_loss_f / len(train_loader), round*args.simclr_e+k) # type: ignore
        sw.add_scalar(f'Train/ls_/{node.num}', running_loss_ls / len(train_loader), round*args.simclr_e+k) # type: ignore
        sw.add_scalar(f'Train/loss/{node.num}', ls / len(train_loader), round*args.simclr_e+k) # type: ignore
        logger.info('Epoch [%d/%d], node %d: Loss_t: %.4f, Loss_f: %.4f, Loss_ls: %.4f, total loss: %.4f' % (k+1, args.simclr_e, node.num, running_loss_t / len(train_loader), running_loss_f / len(train_loader), running_loss_ls / len(train_loader), ls/len(train_loader))) # type: ignore
...
class SampleGenerator(Dataset):
    def __init__(self, num_samples, latent_space, gen_model, classes=10, device='cuda:0'):
        ...
        for cls in range(self.classes):
            ...
            # 假样本
            fake_imgs = self.gen_model(z_, y_)
...
def train_fc(node, args, logger, round, sw=None):
    ...
    # train_loader = node.train_data
    # 假设有测试数据生成器
    train_dataset = SampleGenerator(1000, args.latent_space, node.gen_model, 10, args.device)
